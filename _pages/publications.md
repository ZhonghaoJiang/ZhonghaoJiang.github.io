---
permalink: /pub/
title: "Publications"
author_profile: true
redirect_from: 
  - /pub/
  - /pub.html/
---
<p class="publication-note"><em>* means equal contribution.</em></p>

<h3 class="publication-section-title">Full Papers</h3>

* 📌 <span class="publication-label">[Preprint]</span>
  **Agentic Software Issue Resolution with Large Language Models: A Survey** \
  <u>Zhonghao Jiang</u>, David Lo, Zhongxin Liu \
  [[PDF]](https://arxiv.org/abs/2512.22256) [[Artifacts]](https://github.com/ZhonghaoJiang/Awesome-Issue-Solving)

* <span class="publication-label">[ASE'26]</span>
  **Effective and Efficient Context Retrieval via Partial Dependency Graph for Repository-Level Code Generation** \
  Zhongxin Liu, <u>Zhonghao Jiang</u>, Zhifan Ye, Haoye Wang, Jiakun Liu, Xiaoxue Ren \
  The 41th IEEE/ACM International Conference on Automated Software Engineering (ASE 2026) \
  [[PDF]](https://arxiv.org/pdf/2608.01927v1) [[Code]](https://github.com/ZhonghaoJiang/DyCoder)

* <span class="publication-label">[ASE'26]</span>
  **Doc2Feat-bench: Evaluating Documentation-Driven Feature Addition** \
  <u>Zhonghao Jiang</u>, Le Deng, Jialun Cao, Michael Pradel, Zhongxin Liu \
  The 41th IEEE/ACM International Conference on Automated Software Engineering (ASE 2026) \
  <span class="small">🌟ArXiv Preprint: NoCode-bench: A Benchmark for Evaluating Natural Language-Driven Feature Addition</span> \
  [[PDF]](https://arxiv.org/pdf/2507.18130v4) [[Code]](https://github.com/Doc2Feat-bench/Doc2Feat-bench) [[Leaderboard]](https://nocodebench.org/)

* <span class="publication-label">[ASE'25]</span>
  **Issue Localization via LLM-Driven Iterative Code Graph Searching** \
  <u>Zhonghao Jiang</u>, Xiaoxue Ren, Meng Yan, Wei Jiang, Yong Li, Zhongxin Liu \
  The 40th IEEE/ACM International Conference on Automated Software Engineering (ASE 2025) \
  <span class="small">🌟ArXiv Preprint: CoSIL: Software Issue Localization via LLM-Driven Code Repository Graph Searching</span> \
  [[PDF]](https://arxiv.org/abs/2503.22424) [[Code]](https://github.com/ZhonghaoJiang/CoSIL)

* <span class="publication-label">[TSE'25]</span>
  **DeepVec: State-Vector Aware Test Case Selection for Enhancing Recurrent Neural Network** \
  <u>Zhonghao Jiang</u>, Meng Yan, Li Huang, Weifeng Sun, Chao Liu, Song Sun, David Lo \
  IEEE Transactions on Software Engineering (TSE) \
  [[PDF]](https://ieeexplore.ieee.org/document/10979368) [[Code]](https://github.com/ZhonghaoJiang/DeepVec)

* <span class="publication-label">[Internetware'24]</span>
  **DFEPT: Data Flow Embedding for Enhancing Pre-Trained Model Based Vulnerability Detection** \
  <u>Zhonghao Jiang</u>, Weifeng Sun, Xiaoyan Gu, Jiaxin Wu, Tao Wen, Haibo Hu and Meng Yan \
  Proceedings of the 15th Asia-Pacific Symposium on Internetware \
  [[PDF]](https://arxiv.org/pdf/2410.18479) [[Code]](https://github.com/GCVulnerability/DFEPT)

<h3 class="publication-section-title">Short Papers</h3>
* <span class="publication-label">[ASE'25 Demo]</span>
  **AgentDroid: A Multi-Agent Tool for Detecting Fraudulent Android
  Applications** \
  Ruwei Pan, Hongyu Zhang, <u>Zhonghao Jiang</u>, Ran Hou \
  The 40th IEEE/ACM International Conference on Automated Software Engineering, Tool Demo Track \
  [[PDF]]() [[Code]](https://github.com/Wwstarry/LLM4Fraud)
